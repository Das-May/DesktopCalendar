...
QScopedPointer<LunarCalendar> LunarCalendar::self;
...
//计算是否闰年
bool LunarCalendar::isLoopYear(int year)
{
...
//计算指定年月对应到该月共多少天
int LunarCalendar::getTotalMonthDays(int year, int month)
{
    int countDay = 0;
    int loopDay = isLoopYear(year) ? 1 : 0;
...
//计算指定年月对应星期几
int LunarCalendar::getFirstDayOfWeek(int year, int month)
{
    int week = 0;
...
//计算国际节日
QString LunarCalendar::getHoliday(int month, int day)
{
    int temp = (month << 8) | day;//比特位操作 <<:左移n位   |:按位"或"
    QString strHoliday;
...
//计算二十四节气
QString LunarCalendar::getSolarTerms(int year, int month, int day)
{
...
//计算农历节日(必须传入农历年份月份)
QString LunarCalendar::getLunarFestival(int month, int day)
{
...
//计算农历年 天干+地支+生肖
QString LunarCalendar::getLunarYear(int year)
{
    QString strYear;
    if (year > 1924) {
        int temp = year - 1924;
...
void LunarCalendar::getLunarCalendarInfo(int year, int month, int day,
        QString &strHoliday,
        QString &strSolarTerms,
...
QString LunarCalendar::getLunarInfo(int year, int month, int day, bool yearInfo, bool monthInfo, bool dayInfo)
{
    QString strHoliday, strSolarTerms, strLunarFestival, strLunarYear, strLunarMonth, strLunarDay;

    LunarCalendar::Instance()->getLunarCalendarInfo(year, month, day,
...
//提供农历的  年和月和日
QString LunarCalendar::getLunarYearMonthDay(int year, int month, int day)
{
    return getLunarInfo(year, month, day, true, true, true);
}

//提供农历的  月和日
QString LunarCalendar::getLunarMonthDay(int year, int month, int day)
{
    return getLunarInfo(year, month, day, false, true, true);
}

//提供农历的 日
QString LunarCalendar::getLunarDay(int year, int month, int day)
{
